...
for i in range(0,len(Ri)):
    ...
    plt.title('$V_{C}$' f'Amplitude Bode Plot for {Ri[i]}')
    plt.ylabel('$V_C$ / V')
    plt.xlabel('$f$ / Hz')
    ...
    #setting the plot title and labling axis
    plt.title(f'Phase Bode Plot{Ri[i]}')
    plt.ylabel('$\phi$ / rad')
    plt.xlabel('$f$ / Hz')
...
#setting the plot title and labling axis
plt.title('$V_{C}$ Amplitude Bode Plot')
plt.ylabel('$V_C$ / V')
plt.xlabel('$f$ / Hz')
...
for i in range(0, 3):
    ...
    axs[0].set_title('Measured Amplitude Curve')
    axs[1].plot(f, ampli(f, *thamp[i]), color=colr[i])
    axs[1].set_title('Theoretical Amplitude, Curve')
    axs[2].plot(f, ampli(f, *simamp[i]), color=colr[i], label=f'R= {R[i]} $\\Omega$')
    axs[2].set_title('Simulated Amplitude Curve')

#set subplot separation, create legend, save plot
fig.tight_layout(pad=0.2)
plt.legend()
plt.savefig('Output/Amplitude Comaprison.png', bbox_inches='tight', dpi=300) #save the plot
plt.show()
...
#setting the plot title and labling axis
plt.title('Phase Bode Plot Comparison')
plt.ylabel('$\phi$ / rad')
plt.xlabel('$f$ / Hz')
...
fig.tight_layout(pad=0.2)
plt.legend() #make a plot legend
plt.savefig('Output/Phase Comparison.png', bbox_inches='tight', dpi=300) #save the plot
plt.show()
...
plt.legend() #make a plot legend
plt.savefig('Output/Phase Comparison Closeup.png', dpi=200) #save the plot
plt.show()
